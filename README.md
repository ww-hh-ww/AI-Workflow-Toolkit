...
日常使用推荐同时打开 Claude Code 和 `aiwf ui`：在 Claude Code 中讨论和工作，在 TUI 中查看全局结构、当前阶段、Task 记录、Memory 和 Git 分支，并执行需要人确认的交互操作。
...
4. 如果选择合并，按既定顺序合并 Plan branch，在 base branch 上运行组合验证。
5. 关闭 Plan：
...
派发 Executor、Tester、Reviewer 时，在 prompt 中写明 Task ID 和已绑定的 worktree。
AIWF Hook 会把 Agent 的相对文件、搜索和 Bash 调用持续路由到该 worktree。不要使用
`EnterWorktree` 或 `isolation: worktree`，也不要在 worktree 之间复制 Task 改动；三种
角色需要顺序处理同一份结果。

所有 worktree 通过 Git common directory 找到主工作区中的同一个 `.aiwf` control root。`aiwf status` 会展示全部 active Tasks，并标记当前 worktree 对应的 Task。
Planner 不需要在这些 worktree 之间切换。它使用明确的 Task ID 管理状态；多个 Task
active 时，`aiwf status --prompt` 给出各自的 worktree，Hook 在每次 Agent 工具调用时
重新确认路由。
...
| PreToolUse Read/Glob/Grep | 把 Task 角色的相对读取和搜索持续路由到对应 Plan worktree |
...
这些 Hook 约束 Claude Code 工具，不限制人在终端或编辑器中的改动。没有活动 Task
