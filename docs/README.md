...
- [AIWF 使用教程：从会用到用好](AIWF-USAGE-TUTORIAL.md) — 给第一次拿到系统的用户，覆盖新项目和已有/半成品项目两条使用路径，以及 Goal、Plan、Task、PROJECT-MAP、确认、降级和闭合的基本用法。
...
| `state/goals.json` | CLI（`aiwf state record-quality-brief`） | 目标、合同、架构约束 |
