...
def _ev_records(cwd):
    return json.loads((cwd / ".aiwf" / "artifacts" / "evidence" / "records.json").read_text())["records"]
...
class TestEvidence(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (self.tmp / ".aiwf" / "artifacts" / "evidence" / "records.json").write_text('{"records":[]}')
        # Reset state
    ...
    def test_internal_paths_never_trigger_scope_violation(self):
        ...
        (self.tmp / ".aiwf" / "artifacts" / "evidence" / "records.json").write_text(json.dumps(ev, indent=2))
