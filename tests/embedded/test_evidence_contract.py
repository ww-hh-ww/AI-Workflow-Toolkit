...
def _ev_records(cwd):
    return json.loads((cwd / ".aiwf" / "records" / "evidence.jsonl").read_text())["records"]
...
class TestEvidence(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (self.tmp / ".aiwf" / "records" / "evidence.jsonl").write_text('{"records":[]}')
        # Reset state
    ...
    def test_internal_paths_never_trigger_scope_violation(self):
        ...
        (self.tmp / ".aiwf" / "records" / "evidence.jsonl").write_text(json.dumps(ev, indent=2))
