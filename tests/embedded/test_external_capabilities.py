...
class TestExternalCapabilities(unittest.TestCase):
    ...
    def test_planned_lifecycle_capability_blocks_activation_until_decided(self):
        from aiwf_core.core.task_ledger import activate_task, upsert_task, close_task
        from aiwf_core.core.capabilities import capability_use_blockers
        ...
        # V2: capability_use_blockers is the authoritative check (process contract),
        # not activation_blockers. Task activation uses activation_blockers which
        # no longer includes lifecycle-overlap checks.
        # First, activate without planned capability — should work
        ...
        # Plan use of the lifecycle-overlap capability
        self._run_ok("capability", "plan-use", "skill:tdd")
        blockers = capability_use_blockers(str(self.tmp))
        self.assertTrue(len(blockers) > 0)
        self.assertTrue(any("lifecycle-overlap capability requires explicit Planner decision" in b for b in blockers))

        # Decide on the capability
    ...
    def test_planner_skill_mentions_cannot_override(self):
        # V2: planner-contracts describes Task.md with role-specific requirements sections
        c = (self.tmp/".claude"/"skills"/"aiwf-planner/references/task-contract.md").read_text()
        self.assertIn("executor requirements", c.lower())
        self.assertIn("reviewer requirements", c.lower())
        self.assertIn("tester requirements", c.lower())
        self.assertIn("forbidden write", c.lower())
