...
class TestPlannerAuthorityBoundaries(unittest.TestCase):
    ...
    def test_active_task_freezes_context_write_boundary(self):
        ...
        ctx = self._read("state/state.json")["contexts"][0]
    ...
    def test_scope_violation_event_keeps_original_boundary_snapshot(self):
        ...
        # V2: tasks live in state/tasks.json, not runtime/history/task-ledger.json
        ...
        self._write("state/state.json", state)
        # V2: check_scope no longer checks allowed_write boundary;
        # violations are triggered by forbidden_write patterns from active_context.
        violations = check_and_record_scope_violations(["secrets/key.txt"], context, self.tmp)
        self.assertEqual(violations, ["secrets/key.txt"])
        event = self._read("records/review.jsonl")["scope_violation_events"][0]
        self.assertEqual(event["path"], "secrets/key.txt")
    ...
    def test_failed_testing_cannot_lower_workflow_level_after_task_stops(self):
        ...
        self._write("state/state.json", state)
        testing = self._read("records/testing.jsonl")
        testing["status"] = "failed"
        self._write("records/testing.jsonl", testing)
    ...
    def test_blocking_review_freezes_context_after_active_task_is_closed(self):
        ...
        state["active_task_id"] = None
        self._write("state/state.json", state)
        review = self._read("records/review.jsonl")
        review["result"] = "needs_fix"
        self._write("records/review.jsonl", review)
    ...
    def test_scope_violation_blocks_next_task_activation(self):
        ...
        self.assertTrue(any("scope violation remains recorded" in blocker for blocker in blockers))
