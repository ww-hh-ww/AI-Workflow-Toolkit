...
class TestGovernanceFiles(unittest.TestCase):
    ...
    def test_review_json_allowed(self):
        self._set_scope(["src/"]); _, out = self._scope_check(".aiwf/quality/review.json")
        self.assertNotIn("permissionDecision", out)

    # ── report and assets ──
    def test_report_md_allowed(self):
        self._set_scope(["src/"]); _, out = self._scope_check(".aiwf/reports/闭合报告.md")
        self.assertNotIn("permissionDecision", out)

    def test_project_map_md_allowed(self):
        self._set_scope(["src/"]); _, out = self._scope_check(".aiwf/reports/项目地图.md")
        self.assertNotIn("permissionDecision", out)

    def test_ideas_md_allowed(self):
        self._set_scope(["src/"]); _, out = self._scope_check(".aiwf/reports/ideas.md")
        self.assertNotIn("permissionDecision", out)

    def test_current_state_md_allowed(self):
        self._set_scope(["src/"]); _, out = self._scope_check(".aiwf/reports/当前状态.md")
        self.assertNotIn("permissionDecision", out)
    ...
    def test_internal_snapshot_allowed(self):
        self._set_scope(["src/"])
        _, out = self._scope_check(".aiwf/internal/pre-tool-snapshot.json")
        self.assertNotIn("permissionDecision", out)
    ...
    def test_classify_governance(self):
        ...
        self.assertEqual(classify_file_change(".aiwf/reports/闭合报告.md"), "governance")
        self.assertEqual(classify_file_change(".aiwf/reports/项目地图.md"), "governance")
        self.assertEqual(classify_file_change(".aiwf/reports/ideas.md"), "governance")
    ...
    def test_governance_not_in_scope_violation(self):
        ...
        files = [".aiwf/reports/闭合报告.md", ".aiwf/assets/x.json", ".aiwf/experiment-artifacts/r.md"]
