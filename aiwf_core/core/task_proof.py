...
STRICT_MARKERS = ("## Fixed Contract", "### Proof Standard")
...
def _extract_verification_commands(section: str) -> List[VerificationCommand]:
    ...
    body = rows[1:] if any("command" in cell for cell in header) else rows
...
def read_task_proof_contract(base_dir: str, task: Dict[str, Any]) -> Optional[TaskProofContract]:
    ...
    strict = all(marker in text for marker in STRICT_MARKERS)
    ...
    if "Verification Commands" in proof_section and not commands:
...
def validate_testing_against_task(
    ...
    recorded = [_norm(cmd) for cmd in (testing.get("commands", []) or []) if _norm(cmd)]
    verification_results = [
        item for item in (testing.get("verification_results", []) or [])
        if isinstance(item, dict)
    ]
    result_by_command = {
        _norm(item.get("command", "")): item
        for item in verification_results
        if _norm(item.get("command", ""))
    }
    recorded_set = set(recorded)
    missing = [cmd for cmd in required if _norm(cmd) not in recorded_set]
    missing_results = [cmd for cmd in required if _norm(cmd) not in result_by_command]
    mismatched = [
        cmd for cmd in required
        if _norm(cmd) in result_by_command
        and not bool(result_by_command[_norm(cmd)].get("matched", False))
    ]
    empty_observed = [
        cmd for cmd in required
        if _norm(cmd) in result_by_command
        and not _norm(result_by_command[_norm(cmd)].get("observed", ""))
    ]
    return {
        "strict": True,
