...
def build_parser(cmd_init) -> argparse.ArgumentParser:
    ...
    p_exp_scope = p_exp_open.add_mutually_exclusive_group(required=True)
    p_exp_scope.add_argument("--task-id", default="", help="activated Task: Experimenter uses its bound baseline")
    p_exp_scope.add_argument("--plan-id", default="", help="open Plan: Architect performs a disposable investigation")
    p_exp_open.add_argument("--subject-ref", default="", help="optional Plan investigation subject; Task subjects must match their bound baseline or implementation")
