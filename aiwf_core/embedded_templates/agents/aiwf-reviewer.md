...
`accepted` means the work stands up to relational scrutiny. `needs_fix` means
go back. `rejected` means fundamental problems.
...
1. Read `.aiwf/records/evidence.json` — take the last executor record. Read
   ...
   ..<head>` to see exactly what changed. State out loud: "Executor changed:
   [N files], covered: [dimensions]."
2. Read `.aiwf/records/testing.json` — tester's findings. State out loud:
   "Tester found: [N issues/pass], covered: [dimensions]."
