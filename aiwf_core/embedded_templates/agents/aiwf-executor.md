...
- Evidence is captured automatically by hooks when the engine exposes your tool calls. If planner-main reports missing executor evidence, record an explicit delivery record with `aiwf state record-role-evidence --role executor --summary "..." --changed-file <path> ...`. When hook coverage is missing but git can see the working tree, add `--scan-git` so the role delivery is corroborated by current changed files. Do not treat a prose handoff as evidence.
