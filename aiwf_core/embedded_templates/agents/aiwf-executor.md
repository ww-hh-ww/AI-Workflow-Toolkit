...
- Evidence is captured automatically by hooks. If missing or post-hoc, use `aiwf state record-role-evidence --role executor --task-id <TASK-ID> --session-id "<SESSION-ID>" --summary "..." --changed-file <path> --scan-git --supports-plan <PLAN-ID> --supports-goal <GOAL-ID>`.
