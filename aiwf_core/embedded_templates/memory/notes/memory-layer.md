...
status, or details already clear in the current Goal/Plan/Task/Milestone.

Before adding memory, check whether an existing fact, index entry, or note
should be changed or deleted instead.

At each moment, decide whether memory should stay as-is, change, be deleted, or
receive a new entry. Maintain it only when a source-backed long-term fact
changed. If nothing changed, move on without adding an entry.
